<?php
    ...
    $review = new DebtorsReview($db);

    $collect = new DebtCollect($db);

    $remn = new DebtReminder($db);

    $operator = isset($_GET['s']) ? $_GET['s'] : die();
    $user = isset($_GET['u']) ? $_GET['u'] : die();

    $stmt1 = $control->read($operator, 1);
    $stmt2 = $control->read($operator, 2);
    $stmt3 = $control->read($operator, 3);
    ...
    if ($num > 0) {
        ...
        while ($row = $stmt1->fetch(PDO::FETCH_ASSOC)) {
            ...
            $control_item = array(
                ...
                'review' => $review->readByDC($dc_id, $user)
            );
